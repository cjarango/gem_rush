...
class WinScreen(ScreenBase):
    ...
    def render(self):
        ...
        self.draw_multiline_text_centered(
            "YOU WIN!", self.title_font, (0, 255, 0), self.screen.get_height() // 2 - 100
        )
        self.draw_multiline_text_centered(
            "Congratulations! You have activated the portal and escaped.", 
            self.font, self.COLOR_TEXT, self.screen.get_height() // 2
        )
        self.draw_multiline_text_centered(
            "Press H or ESC to return to the menu", 
            self.font, self.COLOR_TEXT, self.screen.get_height() // 2 + 50
        )
        pygame.display.flip()
